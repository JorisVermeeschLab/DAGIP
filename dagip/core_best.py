...
def ot_da(
    ...
    distances = pairwise_distances(X1_second, X2)
    gamma = torch.FloatTensor(transport_plan(distances.cpu().data.numpy()))

    n_hidden = 32
    adapter = MLPAdapter(X1.size()[0], X1.size()[1], (n_hidden, n_hidden, n_hidden, n_hidden), eta=avg_deviation, transpose=False)
    ...
    for iteration in pbar:
        ...
        #if (iteration % 10 == 0) or (gamma is None):
        #    gamma = torch.FloatTensor(transport_plan(distances.cpu().data.numpy()))
        ...
        loss1 = torch.sum(gamma_ * torch.square(distances_))
